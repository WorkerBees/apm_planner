        ...
        for (i = parameters.begin(); i != parameters.end(); ++i) {
            ...
            QMap<QString, QVariant>* comp = i.value();
            {
                ...
                for (j = comp->begin(); j != comp->end(); ++j)
                {
                    ...
                    QString paramType("%1");
                    switch ((QMetaType::Type)j.value().type())
                    {
                    case QMetaType::Int:
    ...
    if (parameterName.contains(splitToken))
    {
        ...
        for (int i = 0; i < parentItem->childCount(); i++) {
            ...
            if (key == parameterName)
            {
                ...
                parameterItem = child;
                if ((QMetaType::Type)value.type() == QMetaType::Char)
                {
...
void QGCParamWidget::parameterItemChanged(QTreeWidgetItem* current, int column)
{
    if (current && column > 0) {
        ...
        if (map) {
            ...
            switch ((QMetaType::Type)parameters.value(key)->value(str).type())
            {
            case QMetaType::Int:
            {
        ...
        foreach (int component, writeKeys) {
            ...
            foreach (QString key, missingParams->keys()) {
                if (count < retransmissionBurstRequestSize) {
                    ...
                    switch ((QMetaType::Type)parameters.value(component)->value(key).type())
                    {
                    case QMetaType::Int:
                    {
                        QVariant fixedValue(value.toInt());
...
void QGCParamWidget::setParameter(int component, QString parameterName, QVariant value)
{
    ...
    switch ((QMetaType::Type)parameters.value(component)->value(parameterName).type())
    {
    case QMetaType::Char:
    {
